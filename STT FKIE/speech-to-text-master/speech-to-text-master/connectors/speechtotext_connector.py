...
class SpeechtotextConnector:
    ...
    def initialize_speechtotext(self, speechtotext_variant, config_profile, webhook_url):
        ...
        try:
            ...
            # Log an error if the configuration object is empty
            ...
            self.audio_buffer = AudioSegment.silent(duration=0)  # Buffer for incoming audio
            self.current_buffer_start_timestamp = None  # Tracks the timestamp of the buffer's start in ms

            # Initialize or reset the audio queue and worker thread
            self.audio_queue = queue.Queue()
            if self.worker_thread and self.worker_thread.is_alive():
                logger.info("Stopping existing worker thread")
                self.audio_queue.put(None)  # Signal the existing worker thread to exit
    ...
    def process_queue(self):
        while True:
            try:
                ...
                if result:
                    logger.info("Preparing to send transcription result to webhook.")
