...
class SenseVoiceSmall(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.emo_dict = {"unk": 25009, "Happy": 25001, "Sad": 25002, "Angry": 25003, "Neutral": 25004, "Fearful":25005, "Disgust": 25006, "Surprised": 25007}
    ...
    def inference(
        ...
        **kwargs,
        ...
        for i in range(b):
            ...
            # Print individual tokens and decoded text
            # print("Token IDs:", token_int)
            # print("Decoded Text:", text)

            # Extract emotion probabilities for ALL emotions in emo_dict
            emotion_probs = ctc_logits[i, : encoder_out_lens[i].item(), :]

            # Initialize dictionary to store probabilities of all emotions
            all_emotions = {emo: 0 for emo in self.emo_dict.keys()}

            for j in range(emotion_probs.shape[0]):  # Iterate over each frame
                for emotion, token_id in self.emo_dict.items():  
                    prob = torch.nn.functional.softmax(emotion_probs[j], dim=-1)[token_id].item()
                    all_emotions[emotion] += prob  # Accumulate probability weight

            # Normalize emotion probabilities
            total_prob = sum(all_emotions.values())
            if total_prob > 0:
                all_emotions = {k: v / total_prob for k, v in all_emotions.items()}  # Normalize to sum 1

            # Extract the dominant emotion
            dominant_emotion = max(all_emotions, key=all_emotions.get)
            dominant_score = all_emotions[dominant_emotion]
            ...
            if output_timestamp:
                ...
                result_i = {
                    ...
                    "text": text,
                    "timestamp": timestamp,
                    "possible_emotions": all_emotions,  # Store as dictionary
                    "dominant_emotion": dominant_emotion,  # Add dominant emotion here
                    "dominant_score": dominant_score,      # Add dominant emotion score
                }
