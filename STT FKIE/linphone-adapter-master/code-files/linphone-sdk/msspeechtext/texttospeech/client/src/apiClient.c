...
texttospeech_apiClient_t *texttospeech_apiClient_create() {
    texttospeech_apiClient_t *apiClient = malloc(sizeof(texttospeech_apiClient_t));
    apiClient->basePath = strdup("http://localhost:8082/texttospeech");
...
texttospeech_sslConfig_t *texttospeech_sslConfig_create(const char *clientCertFile, const char *clientKeyFile, const char *CACertFile, int insecureSkipTlsVerify) {
    texttospeech_sslConfig_t *sslConfig = calloc(1, sizeof(texttospeech_sslConfig_t));
    if ( clientCertFile ) {
...
void texttospeech_apiClient_invoke(texttospeech_apiClient_t    *apiClient,
                      const char    *operationParameter,
                      list_t        *queryParameters,
...
size_t writeDataCallback(void *buffer, size_t size, size_t nmemb, void *userp) {
    ...
    texttospeech_apiClient_t *apiClient = (texttospeech_apiClient_t *)userp;
