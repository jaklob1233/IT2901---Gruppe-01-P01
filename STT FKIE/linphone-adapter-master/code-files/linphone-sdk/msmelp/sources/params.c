...
params_t* params(int id) {
    ...
    if (stat(fname, &s) == 0) {
        ...
        memset(singleton, 0, sizeof(singleton));
        ms_message("MELP - could not open config file");
    }
...
static void parse(const char* fname) {
    ...
    if (!file) {
        ms_message("MELP - could not read config file");
        return;
    }
    ms_message("MELP - config file read");
    ...
    while (getline(&line, &size, file) != -1) {
        {
            ...
            if (ret == 1) {
                if (strcasecmp(mime, "MELP600") == 0)
                    p = &singleton[0];
                else if (strcasecmp(mime, "MELP1200") == 0)
                    p = &singleton[1];
                else if (strcasecmp(mime, "MELP2400") == 0)
                    p = &singleton[2];
                else
                    (void)0;
...

        {
            ...
            if (strcasecmp(key, "fin") == 0)
                ...
                p->npp = strcasecmp(value, "true") == 0;
            else if (strcasecmp(key, "pf") == 0)
                p->pf = strcasecmp(value, "true") == 0;
            else
                (void)0;
