...
using namespace org::openapitools::speechtotext::server::helpers;
using namespace org::openapitools::speechtotext::server::model;

const std::string DefaultApi::base = "/speechtotext";
...
void DefaultApi::setupRoutes() {
    ...
    Routes::Post(*router, base + "/on_new_text", Routes::bind(&DefaultApi::on_new_text_post_handler, this));
    Routes::Post(*router, base + "/provide_audio", Routes::bind(&DefaultApi::provide_audio_handler, this));
...
std::pair<Pistache::Http::Code, std::string> DefaultApi::handleParsingException(const std::exception& ex) const noexcept
{
    ...
    } catch (org::openapitools::speechtotext::server::helpers::ValidationException &e) {
        return std::make_pair(Pistache::Http::Code::Bad_Request, e.what());
...
void DefaultApi::initialize_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    ...
    auto speechtotextVariantQuery = request.query().get("speechtotext_variant");
    std::optional<std::string> speechtotextVariant;
    if(speechtotextVariantQuery.has_value()){
        std::string valueQuery_instance;
        if(fromStringValue(speechtotextVariantQuery.value(), valueQuery_instance)){
            speechtotextVariant = valueQuery_instance;
        }
    }
    ...
    try {
        this->initialize(speechtotextVariant, configProfile, webhookUrl, response);
...
void DefaultApi::on_new_text_post_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    try {


    // Getting the body param
    
    _on_new_text_post_request onNewTextPostRequest;
    
    try {
        nlohmann::json::parse(request.body()).get_to(onNewTextPostRequest);
        onNewTextPostRequest.validate();
    } catch (std::exception &e) {
    ...
    try {
        this->on_new_text_post(onNewTextPostRequest, response);
    } catch (Pistache::Http::HttpError &e) {
        response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
        return;
    } catch (std::exception &e) {
...
void DefaultApi::provide_audio_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    try {


    // Getting the body param
    
    Provide_audio_request provideAudioRequest;
    
    try {
        nlohmann::json::parse(request.body()).get_to(provideAudioRequest);
        provideAudioRequest.validate();
    } catch (std::exception &e) {
        this->handleParsingException(e, response);
        return;
    }
    ...
    try {
        this->provide_audio(provideAudioRequest, response);
    } catch (Pistache::Http::HttpError &e) {
        response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
        return;
    } catch (std::exception &e) {
