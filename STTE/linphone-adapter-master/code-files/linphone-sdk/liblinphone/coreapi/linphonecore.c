...
#include <speechtotext/api-server.h>
#include <texttospeech/api-server.h>
#include <mqtttransport/api-server.h>
...
void *start_speechtotext_webhook_wrapper(void *arg) {
    (void)arg;  // Avoid unused parameter warning
    ms_message("start speechtotext-webhook-server");
    org::openapitools::speechtotext::server::api::start_server();  // Call the actual server function
    ms_message("speechtotext-webhook-server started");
    return nullptr;
}

void *start_texttospeech_webhook_wrapper(void *arg) {
    (void)arg;  // Avoid unused parameter warning
    ms_message("start texttospeech-webhook-server");
    org::openapitools::texttospeech::server::api::start_server();  // Call the actual server function
    ms_message("texttospeech-webhook-server started");
    return nullptr;
}

void *start_mqtttransport_webhook_wrapper(void *arg) {
    (void)arg;  // Avoid unused parameter warning
    ms_message("start mqtttransport-webhook-server");
    org::openapitools::mqtttransport::server::api::start_server();  // Call the actual server function
    ms_message("mqtttransport-webhook-server started");
    return nullptr;
}
...
static void linphone_core_register_default_codecs(LinphoneCore *lc) {
    ...
    linphone_core_register_payload_type(lc, &payload_type_lyra_3200, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_lyra_6000, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_lyra_9200, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_speechtext, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_melp_600, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_melp_1200, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_melp_2400, NULL, FALSE);
    linphone_core_register_payload_type(lc, &payload_type_melpwb, NULL, FALSE);
...
static void linphone_core_init(LinphoneCore *lc,
    ...
    // Text-to-speech - start server for Webhooks for STT, TTS and MQTT
    // start STT
    pthread_t server_thread1;
    if (pthread_create(&server_thread1, nullptr, start_speechtotext_webhook_wrapper, nullptr) != 0) {
        perror("Failed to create speech-to-text webhook server thread");
        return;
    }
    pthread_detach(server_thread1); // Avoid blocking Linphone's main thread

    // start TTS
    pthread_t server_thread2;
    if (pthread_create(&server_thread2, nullptr, start_texttospeech_webhook_wrapper, nullptr) != 0) {
        perror("Failed to create text-to-speech webhook server thread");
        return;
    }
    pthread_detach(server_thread2); // Avoid blocking Linphone's main thread

    // start MQTTTRANSPORT
    pthread_t server_thread3;
    if (pthread_create(&server_thread3, nullptr, start_mqtttransport_webhook_wrapper, nullptr) != 0) {
        perror("Failed to create mqtttransport webhook server thread");
        return;
    }
    pthread_detach(server_thread3); // Avoid blocking Linphone's main thread
...
LinphoneAccount *linphone_core_lookup_account_by_identity_strict(LinphoneCore *lc, const LinphoneAddress *uri) {
	...
	for (elem = linphone_core_get_account_list(lc); elem != NULL; elem = elem->next) {
		LinphoneAccount *acc = (LinphoneAccount *)elem->data;
		if (linphone_address_weak_equal(
		        uri, linphone_account_params_get_identity_address(linphone_account_get_params(acc)))) {
			if (linphone_account_get_state(acc) == LinphoneRegistrationOk) {
				found_acc = acc;
				break;
			} else if (!found_reg_acc &&
...
void sip_config_uninit(LinphoneCore *lc) {
	...
	config->deleted_accounts =
	    bctbx_list_free_with_data(config->deleted_accounts, (void (*)(void *))linphone_account_unref);
	config->deleted_proxies =
	    bctbx_list_free_with_data(config->deleted_proxies, (void (*)(void *))_linphone_proxy_config_release);

	/*no longuer need to write proxy config if not changed
