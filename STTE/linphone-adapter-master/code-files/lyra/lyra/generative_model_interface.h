...
// replaced glog/logging by iostream
#include <iostream>
 ...
 public:
  ...
  bool AddFeatures(const std::vector<float>& features) override final {
    if (features.size() != num_features_) {
      std::cout << "Expecting features to be of shape " << num_features_
  ...
  std::optional<std::vector<int16_t>> GenerateSamples(
    ...
    if (num_samples < 0) {
      std::cout  << "Number of samples must be positive.";
      return std::nullopt;
    }
    ...
    if (num_samples_available() == 0) {
       std::cout << "Tried generating " << num_samples << " samples but only "
